...
if menu == "總覽 (Dashboard)":
    ...
    with col2:
        ...
        if not df_latest.empty:
            ...
            st.dataframe(
                df_sorted[['ID', 'Name', 'Weight_str']], 
                use_container_width=True,
                height=400,
                hide_index=True,
                key="dashboard_weight_table" # 增加 key
            )

    # 需求1: 標題改名 & 需求4: 避免殘影
    st.divider()
    st.subheader("⚠️ 股價跌破ETF成本線") # 改名
    ...
    if st.session_state.get('run_dashboard_analysis'):
        ...
        if not df_underwater.empty:
            ...
            event = st.dataframe(
                ...
                key="dashboard_underwater_table" # 增加 key 避免殘影
            )
            ...
            if len(event.selection.rows) > 0:
                ...
                st.divider()
                # 傳入 dashboard 作為 prefix，確保 key 唯一
                draw_analysis_chart(selected_row['代號'], selected_row['名稱'], df_files, "dashboard")
        else:
...
# =========================================================
    ...
    with col_date:
        pick_date = st.date_input("選擇日期", latest_date_record.date(), key="daily_date_picker") # 增加 key
        ...
        if prev_idx < 0:
            ...
            for _, row in df_change.iterrows():
                ...
                display_rows.append({
                    ...
                    '股數變化': int(row['股數變化']), '差額': diff_txt, 'Raw_Diff': row['股數變化']
                })
            ...
            if not df_display.empty:
                ...
                st.dataframe(
                    ...
                    use_container_width=True,
                    key="daily_change_table" # 增加 key 避免殘影
                )
                ...
                target_label = st.selectbox(
                    "選擇股票:", 
                    df_display['股票代號'] + " " + df_display['股票名稱'],
                    key="daily_stock_selector" # 增加 key
                )
